          ...
          <div className="px-10 py-7">
            {step === 1 && (
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={project} onChange={e => setProject(e.target.value)} placeholder="例：［2026-36］瓦基 AI" className="w-full bg-white border-[1.5px] border-[#e2e0db] rounded-lg text-[14px] px-4 py-3 outline-none focus:border-[#1a56db] transition-colors placeholder:text-[#9a9590]" />
                </div>
                <div>
                  <label className="block font-mono text-[10px] text-[#9a9590] tracking-[0.08em] uppercase mb-1.5">負責人</label>
                  <input type="text" value={owner} onChange={e => setOwner(e.target.value)} placeholder="例：Alan" className="w-full bg-white border-[1.5px] border-[#e2e0db] rounded-lg text-[14px] px-4 py-3 outline-none focus:border-[#1a56db] transition-colors placeholder:text-[#9a9590]" />
                </div>
            ...
            {step === 2 && (
              <div className="flex flex-col gap-4">
                ...
                <textarea value={raw} onChange={e => setRaw(e.target.value)} placeholder={'WEBVTT\n\n00:00:01.000 --> 00:00:04.000\n大家好歡迎來到今天的課程...'} className="w-full bg-[#f6f5f3] border-[1.5px] border-[#e2e0db] rounded-lg font-mono text-[12px] px-4 py-3 h-32 resize-none outline-none focus:border-[#1a56db] leading-relaxed placeholder:text-[#9a9590]" />
              </div>
            )}
            {step === 3 && (
              <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-2">
                  <input type="text" value={termInput} onChange={e => setTermInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && addTerm()} placeholder="如：Final Cut Pro、林小明老師..." className="flex-1 bg-white border-[1.5px] border-[#e2e0db] rounded-lg text-[13px] px-4 py-3 outline-none focus:border-[#1a56db] transition-colors placeholder:text-[#9a9590]" />
                ...
                {corrected && (
                  <>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {([['原始行數', origLines], ['校正後行數', newLines], ['格式', 'VTT']] as [string, string|number][]).map(([l, v]) => (
                    ...
                    <div className="flex bg-[#f6f5f3] border border-[#e2e0db] rounded-lg p-1 gap-0.5">
                      {(['result', 'original'] as const).map(t => (
                        <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-2 text-[12px] font-medium rounded-md transition-all ${activeTab === t ? 'bg-white text-[#111010] shadow-sm' : 'text-[#9a9590]'}`}>
                          {t === 'result' ? '校正結果' : '原始內容'}
                        </button>
                      ))}
                    </div>
          ...
          <div className="px-10 py-5 bg-[#f6f5f3] border-t border-[#e2e0db] flex items-center justify-between">
            {step === 4 ? (
              <>
                ...
                <div className="flex gap-2">
                  <button onClick={() => navigator.clipboard.writeText(corrected)} className="bg-white border-[1.5px] border-[#e2e0db] rounded-lg text-[13px] font-medium text-[#58544e] px-5 py-2.5 hover:border-[#c9c6c0] transition-all">複製</button>
                  <button onClick={download} className="bg-[#1a56db] border-none rounded-lg text-[13px] font-medium text-white px-5 py-2.5 hover:bg-[#1648c2] transition-all">下載 .vtt</button>
                </div>
              </>
            ) : (
              <>
                {step > 1 ? <button onClick={() => setStep(s => s - 1)} className="text-[13px] text-[#9a9590] hover:text-[#111010] transition-colors">← 上一步</button> : <span />}
                <button onClick={() => step < 3 ? setStep(s => s + 1) : runCorrection()} disabled={loading} className="bg-[#111010] border-none rounded-lg text-[14px] font-medium text-white px-6 py-3 hover:bg-[#2d2a27] transition-all disabled:bg-[#c9c6c0] disabled:cursor-not-allowed">
